...
Before running, update the BULK INSERT source path inside Staging.sp_Load_Sales_Data to point at your local copy of Sales.csv.
What I'd Do Differently at Scale
•	Incremental loads instead of full truncate/delete-and-reload — works fine at 675K rows, but wouldn't scale to real production volume.
•	Move BULK INSERT off a local file path to blob or network storage, so ingestion isn't tied to one machine.
•	A real orchestrator (SQL Agent, Azure Data Factory, or Airflow) instead of manually running staging and normalization as two separate calls.
•	Partition fact.FactSales by date if historization or much larger volumes come into scope.
Technology Stack
